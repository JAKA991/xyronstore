...
const uploadProduct = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      cb(null, "product-" + Date.now() + "-" + file.originalname.replace(/\s+/g, "-"));
    }
  })
});
...
const uploadPayment = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      cb(null, "payment-" + Date.now() + "-" + file.originalname.replace(/\s+/g, "-"));
    }
  })
});
...
function requireLogin(req, res, next) {
  ...
  if (user.status !== "active") {
    req.session.destroy(() => res.redirect("/JakSkyLogin"));
    return;
  }
